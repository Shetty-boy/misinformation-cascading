...
> The current pipeline runs on a single machine via Spark's local mode. The
> architecture does not preclude scaling to a genuine multi-node Spark cluster
> (e.g. via distributed storage such as HDFS) if larger-than-single-machine cascade
> volumes are needed later; this is left as a possible extension rather than
